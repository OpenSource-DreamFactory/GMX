# GMX Liquidator keeper 

this keeper will automatically liquidate unhealthy positions in the gmx contracts.  It can either be run as a single container, connecting to an external MongoDB database, or the database and keeper can be run together with `docker-compose`.
...
```
docker build -t liquidatorKeeper .
docker run --env-file .env liquidatorKeeper
```
